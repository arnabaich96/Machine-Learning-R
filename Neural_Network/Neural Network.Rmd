...
```{r setup}
packages = c("ggplot2","gridExtra","plotly","kableExtra","neuralnet","here","readr", "ggplot2", "keras", "png", "colorspace", "imager", "SpatialPack", "dplyr", "tidyverse", "Thermimage","reticulate","tensorflow","torchvision","torch")
...
img <- readPNG(here("Datasets/horse001b.png"))
...
model <- keras_model_sequential() %>%
  layer_dense(units = length(train_labels), activation = "relu", input_shape = c(dim(train_data)[2])) %>%
...
model %>% compile(
  ...
  optimizer = optimizer_sgd(learning_rate = learning_rate_schedule_inverse_time_decay(
    initial_learning_rate = 0.01, decay_steps = 100, decay_rate = 0.5))
)
...
```{r,tidy=TRUE, tidy.opts=list(width.cutoff=60)}
ggplotly(ggplot()+geom_line(aes(x=seq(1,history$params$epochs),y=history$metrics$loss),color = 'blue',linetype=1,size=1.2)+xlab('Epoch')+ylab('Loss'))
# output image
output = mirror.matrix(t(matrix(predict(model,train_data),ncol= 128)))
image(output,axes = FALSE,col = grey(seq(0, 1, length = 255)))
```
